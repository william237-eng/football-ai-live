"""
prediction_registry_worldcup.py
=============================
Registre persistant des prédictions émises pour la page Coupe du Monde.
Fichier : database/prediction_registry_worldcup.json
"""
...
REGISTRY_PATH = os.path.join(
    ...
    "database", "prediction_registry_worldcup.json"
)
...
def register_prediction(match_data: Dict[str, Any], market: str) -> bool:
    """
    Enregistre une prédiction si elle n'existe pas encore.
    clef unique: f"{fixture_id}_{market}" pour permettre plusieurs marchés par match.
    ...
    data["predictions"][key] = {
        ...
        "probability":          match_data.get("prob", match_data.get("pct", 0.0)),
...
def prediction_exists(fixture_id: int, market: str) -> bool:
    key = f"{fixture_id}_{market}"
    data = _load()
    return key in data["predictions"]


def get_prediction_key(fixture_id: int, market: str) -> Optional[str]:
    key = f"{fixture_id}_{market}"
    data = _load()
    return key if key in data["predictions"] else None


def get_prediction(fixture_id: int, market: str) -> Optional[Dict[str, Any]]:
    key = f"{fixture_id}_{market}"
    data = _load()
    return data["predictions"].get(key)


def validate_prediction(fixture_id: int, market: str, result: str, details: Any = None) -> bool:
    """
...
def get_predictions_by_market(market: str) -> List[Dict[str, Any]]:
    return [p for p in get_all_predictions() if p.get("market") == market]
...
def compute_real_stats(market: str = None, days: int = 30) -> Dict[str, Any]:
    ...
    if market:
        all_preds = [p for p in all_preds if p.get("market") == market]
