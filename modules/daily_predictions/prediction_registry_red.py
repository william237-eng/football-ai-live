"""
prediction_registry_red.py
================================
Registre persistant des prédictions émises pour les cartons rouges (>=1 red card).
Fichier : database/prediction_registry_red.json
"""
...
REGISTRY_PATH = os.path.join(
    ...
    "database", "prediction_registry_red.json"
)
...
def register_prediction(match_data: Dict[str, Any]) -> bool:
    ...
    data["predictions"][key] = {
        ...
        "prediction":           "RED_CARD_>=1",
        "probability":          match_data.get("prob", 0.0),
...
def validate_prediction(fixture_id: int, result: str, total_reds: int) -> bool:
    key = str(fixture_id)
    data = _load()
    ...
    pred["result"] = result
    pred["total_reds_final"] = total_reds
